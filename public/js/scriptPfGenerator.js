...
function showQuestion(){
	...
	while (questionTab.firstChild)
		questionTab.removeChild(questionTab.lastChild);
...
function showQuizResult(){
	...
	$("#finishQuestButtonsDiv").css("display", "flex");

	drawAssetMixGraph()
	$("#finishQuestButtonsDiv").show();
}

function drawAssetMixGraph(){
	var data = [];
	var labels = ["Stocks","Bonds"];
	if( riskScore < 10 ){
		//100 bond 0 stocks
		data = [100,0];
	}
	...
	$('#assetMixCanvas').remove();
	$('#assetMixDiv').append('<canvas id="assetMixCanvas"><canvas>');
...
function retakeQuest(){
	$(".assetMixLabel").hide();
	$("#finishQuestButtonsDiv").hide();
	$("#assetMixCanvas").hide();
	responses = [];
	actualQuestion = 0;
	riskScore = 0;
